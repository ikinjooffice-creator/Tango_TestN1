(() => {
  ...
  const el = {
    ...
    deckPosition: $('deckPosition'), timerState: $('timerState'), progressFill: $('progressFill'), controlsToggle: $('controlsToggle'), controlsPanel: $('controlsPanel'), restartBtn: $('restartBtn'), prevBtn: $('prevBtn'), pauseBtn: $('pauseBtn'), nextBtn: $('nextBtn'),
  ...
  function showControls() {
    if (!el.controlsPanel || !el.controlsToggle) return;
    el.controlsPanel.classList.remove('collapsed-controls');
    el.controlsToggle.setAttribute('aria-expanded', 'true');
    resetControlsAutoHide();
  }
  function hideControls() {
    if (!el.controlsPanel || !el.controlsToggle) return;
    el.controlsPanel.classList.add('collapsed-controls');
    el.controlsToggle.setAttribute('aria-expanded', 'false');
  ...
  function toggleControls() {
    ...
    if (el.controlsPanel.classList.contains('collapsed-controls')) showControls();
    else hideControls();
  }
  function resetControlsAutoHide() {
    if (state.controlsHideTimer) clearTimeout(state.controlsHideTimer);
    state.controlsHideTimer = setTimeout(() => hideControls(), 4500);
  }

  function bind() {
    el.openSettings.addEventListener('click', () => el.settingsPanel.classList.toggle('collapsed'));
    if (el.controlsToggle) el.controlsToggle.addEventListener('click', toggleControls);
